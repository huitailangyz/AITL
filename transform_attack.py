...
def build_attack_graph_single_step(g, params):
    eps = 2.0 * params['max_epsilon'] / 255.0
    num_classes = 1001
    batch_shape = [params['batch_size'], params['image_height'], params['image_width'], 3]
    sign_flag = 1.0 if params['untarget'] else -1.0

    model_list = params['model_list'].split('_')
    ...
    def graph(x, noise_x, y, x_max, x_min, grad, variance, op_chosen):
        eps = 2.0 * params['max_epsilon'] / 255.0
    ...
    with g.as_default():
            ...
            noise_x_input = tf.placeholder(tf.float32, shape=[params['batch_size'], params['num_noise'], params['image_height'], params['image_width'], 3])
            x_max = tf.clip_by_value(x_input + eps, -1.0, 1.0)
            x_min = tf.clip_by_value(x_input - eps, -1.0, 1.0)

            x_labels = tf.placeholder(tf.int32, shape=[params['batch_size']])
            op_chosen = tf.placeholder(tf.int32, shape=[params['batch_size'], params['num_noise'], params['encoder_length']])
            y = tf.one_hot(x_labels, num_classes)

            grad = tf.placeholder(tf.float32, shape=batch_shape)
            variance = tf.placeholder(tf.float32, shape=batch_shape)

            x_adv, grad_adv, variance_adv = graph(x_input, noise_x_input, y, x_max, x_min, grad, variance, op_chosen)

            return x_input, noise_x_input, x_labels, op_chosen, grad, variance, x_adv, grad_adv, variance_adv